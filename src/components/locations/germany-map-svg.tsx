...
export function GermanyMapSvg({ className = "" }: { className?: string }) {
  ...
  const dots = [
    { id: 0, cx: 846, cy: 336, delay: "0s" },    // Berlin
    ...
    { id: 11, cx: 624, cy: 188, delay: "1.1s" }, // Mecklenburg-Vorpommern (Schwerin)
  ...
  const connections = [
    [0, 10], [0, 11], [0, 9], // Berlin to Brandenburg, MV, Sachsen-Anhalt
    [10, 7], [10, 11], // Brandenburg to Sachsen, MV
    [7, 8], [7, 9], // Sachsen to Thüringen, Sachsen-Anhalt
    [9, 8], [9, 6], [9, 11], // Sachsen-Anhalt to Thüringen, Niedersachsen, MV
    [6, 1], [6, 13], [6, 3], [6, 8], // Niedersachsen to Hamburg, Bremen, NRW, Thüringen
    [1, 12], [1, 11], [1, 13], // Hamburg to SH, MV, Bremen
    [12, 11], // SH to MV
    [8, 4], [8, 2], // Thüringen to Hessen, Bayern
    [3, 4], [3, 14], // NRW to Hessen, Rheinland-Pfalz
    [4, 14], [4, 5], [4, 2], // Hessen to Rheinland-Pfalz, BW, Bayern
    [14, 15], [14, 5], // Rheinland-Pfalz to Saarland, BW
    [5, 2], // BW to Bayern
  ];

  return (
    <svg
      viewBox="0 -100 1024 1200"
      fill="none"
    ...
    >
      <defs>
        ...
        <style>
          {`
            @keyframes pulse-dot {
              0% { transform: scale(1); opacity: 1; filter: drop-shadow(0 0 12px #FBBF24); }
              50% { transform: scale(1.15); opacity: 0.85; filter: drop-shadow(0 0 25px #FDE68A); }
              100% { transform: scale(1); opacity: 1; filter: drop-shadow(0 0 12px #FBBF24); }
            }
            .pin-pulse {
              animation: pulse-dot 6s ease-in-out infinite;
              transform-origin: 0px 15px; /* Anchor the scale precisely at the pin tip (0, 15) */
            }
            @keyframes map-float {
      ...
      <g className="animate-float">
          ...
          <path
            ...
            className="drop-shadow-[0_0_40px_rgba(251,191,36,0.15)]"
            transform="translate(0, 1024) scale(0.1, -0.1)"
          />
        
        {/* Connecting Web Lines */}
        {connections.map((conn, i) => {
          const fromDot = dots.find(d => d.id === conn[0])!;
          const toDot = dots.find(d => d.id === conn[1])!;
          return (
            <line
              key={`conn-${i}`}
              x1={fromDot.cx}
        ...
        {dots.map((dot, i) => (
          <g key={i} transform={`translate(${dot.cx}, ${dot.cy})`}>
            ...
            >
              {/* Premium map pin path matching modern map designs */}
              <path
                d="M0 -15 C-5.5 -15 -10 -10.5 -10 -5 C-10 2.5 0 15 0 15 C0 15 10 2.5 10 -5 C10 -10.5 5.5 -15 0 -15 Z"
                fill="#FBBF24"
              />
