      ...
      <div className="container relative px-4 sm:px-6">
        ...
        <div className="text-center mb-8 sm:mb-16">
          <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full bg-navy-50 border border-navy-100 px-3 py-1.5 sm:px-4 sm:py-2 mb-4 sm:mb-6">
            ...
            <span className="text-xs sm:text-sm font-medium text-navy-700">Deutschlandweit</span>
          </div>
          <h2 className="text-2xl sm:text-3xl lg:text-5xl font-bold mb-3 sm:mb-4 text-navy-900">{t("title")}</h2>
          <p className="text-sm sm:text-lg text-slate-600 max-w-2xl mx-auto px-2">{t("description")}</p>
        </div>

        {/* Location Cards Grid - 2 cols on mobile */}
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4">
          {germanStates.map((state, index) => (
            <Link
              key={state.slug}
              href={`/standorte/${state.slug}`}
              className="group relative bg-white rounded-xl sm:rounded-2xl border border-slate-100 p-3 sm:p-5 hover:border-gold-300 hover:shadow-lg transition-all duration-300 active:scale-[0.98] sm:active:scale-100"
            ...
            >
              ...
              <div className="relative sm:hidden flex flex-col items-center text-center gap-2">
                <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-navy-50 to-navy-100 flex items-center justify-center group-hover:from-gold-100 group-hover:to-gold-200 transition-colors duration-300">
                  <MapPin className="h-4 w-4 text-navy-600 group-hover:text-gold-600 transition-colors" />
                </div>
        ...
        <div className="text-center mt-8 sm:mt-12">
          <Link 
            href="/standorte" 
            className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-semibold text-sm sm:text-base text-navy-900 bg-white border-2 border-navy-200 hover:border-gold-400 hover:shadow-lg transition-all group active:scale-[0.98]"
          >
            {t("viewAll")}
